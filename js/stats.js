 ...
 * ===================================================================*/
import { DAY_SECONDS, RES_INFO, RECIPES, isWinterDay, seasonOf, SEASON_DAYS, YEAR_DAYS, traitOf } from './config.js';
import { ICONS } from './icons.js';
import { G, houseCapacity } from './world.js';
import { productionPerDay } from './sim.js';
...
function flowPerDay() {
  ...
  const prod = {};                                    // res -> 每日预期产量
  const use = {};                                     // res -> 每日预期消耗
  ...
  for (const p of G.placed) {
    ...
    for (const [r, v] of Object.entries(rc.in)) use[r] = (use[r] || 0) + (starved ? 0 : v * cycles);
    for (const [r, v] of Object.entries(rc.out)) prod[r] = (prod[r] || 0) + (starved ? 0 : v * cycles);
  }
