...
import { CELL_SINK, DRAG_TOLERANCE, RES_INFO } from './config';
import { mainEl, dom, cam, camCtl, buildGrid, raycaster, groundPlane, toGround, pickAt } from './scene';
import { G, footprint, cellsOf, canPlace, clampCell, canAfford, pay } from './world';
import { makePad, showGhost, hideGhost, updateGhost, createSite, ghostOK, ghostBad } from './buildings';
import { command } from './villagers';
import { highlightNode } from './nature';
...
export function startPlacing(def) {
  ...
  (document.getElementById('b-' + def.id) as HTMLElement).classList.add('sel');
...
const selrect = document.getElementById('selrect') as HTMLElement;
...
(document.getElementById('zin') as HTMLElement).onclick  = () => { camCtl.r = Math.max(7, camCtl.r - 4); camCtl.apply(); };
(document.getElementById('zout') as HTMLElement).onclick = () => { camCtl.r = Math.min(55, camCtl.r + 4); camCtl.apply(); };
