...
import { DAY_SECONDS, RES_INFO, CARRY_CAP, DEFS, CATS, RECIPES, TECHS, MILESTONES, seasonOf, SEASON_DAYS, YEAR_DAYS, TRADE, EVENTS, isMarketDay } from './config';
import { spawnVillagers } from './villagers';
import { ICONS } from './icons';
const ics = r => `<span class="ics">${ICONS[r] || ''}</span>`;
import { mainEl, camCtl, pickAt } from './scene';
import { G, canAfford, unlocked, houseCapacity } from './world';
import { protos } from './assets';
import { carryTotal } from './drops';
...
export const UI: any = {
  ...
  pumpThumb() {
    ...
    if (img) (img as HTMLImageElement).src = r.domElement.toDataURL();
    sc.remove(obj);
  ...
  showBuildingInfo(entry) {
    ...
    el.querySelectorAll('button[data-tr]').forEach((b: any) => b.onclick = () => {
  ...
  renderPeople() {
    ...
    el.querySelectorAll('.prow').forEach((row: any) => row.onclick = () => {
